...
/**
 * Used to query {@link MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI} and
 * return the artists on a user's device.
 * 
 * @author Kamil Tollinger (kamil.tollinger@gmail.com)
 */
public class AlbumArtistLoader extends WrappedAsyncTaskLoader<List<Artist>> {
     ...
     */
    public AlbumArtistLoader(final Context context) {
        super(context);
    }
    ...
    public static final Cursor makeArtistCursor(final Context context) {
        return context.getContentResolver().query(MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI,
                ...
                }, "_id IS NOT NULL) GROUP BY (artist", null, PreferenceUtils.getInstance(context).getArtistSortOrder());
    }
}
